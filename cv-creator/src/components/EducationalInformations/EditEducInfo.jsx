...
export function EditEducInfo({
  ...
  const formattedStartDate = `${rawStartDate.toLocaleString('default', { month: 'short' })} ${rawStartDate.getFullYear()}`
  const rawEndDate = new Date(educationEndDate)
  const formattedEndDate = `${rawEndDate.toLocaleString('default', { month: 'short' })} ${rawEndDate.getFullYear()}`
  return (
    <>
